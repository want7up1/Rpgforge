...
### 第一梯队 — 低成本、立竿见影（建议优先）
- [ ] **B1 结局闭环**：末幕完成 → `game.status="completed"` → 复用 GM 生成 epilogue → 前端「剧终」卡片。（最刺眼、成本最低）
- [ ] **C1 开局序章** + **C2 play 页目标条**：生成时多产一段开场；侧栏固定显示当前幕目标 + 进行中任务（数据现成）。
- [ ] **C3 首次引导卡**：一次性解释四种输入模式 +「你可以尝试任何行动」。
