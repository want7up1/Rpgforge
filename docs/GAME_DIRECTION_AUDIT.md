...
- [x] **A1 轻量判定层**（Round 34）：StoryDirector 标 difficulty/属性/技能/社交对象 → 后端 action_resolver roll d20 + 修正 vs DC → outcome 作硬约束喂 GM。**A2–A5 由此获得意义。**
- [x] **A2 数值反哺**（Round 34）：等级/技能(level+mastery)/属性(D&D 调整值)/关系(trust·affection)全部进判定修正（关系即社交软门槛）。
- [x] **B3 压力时钟兑现** + **A3 危机条/失败出口**（Round 34）：survival_clock 每回合推进压力时钟，阈值触发侵蚀危机条；危机条受失败/状态/压力侵蚀，归零触发 `defeated` 失败结局（复用 B1 闭环）。
- [x] **B5 松绑校验器**（Round 34）：drift_validator 规则 4b「玩家来源豁免」，把「忠于剧本」收窄到「不提前剧透真相」。
- [x] **C5 重 roll**（Round 34，生成结果区「重新生成」）+ **C6 后悔药**（Round 34，回退到第 N 回合 endpoint + play 页「撤销上一回合」；每回合持久化=自动存档）。
