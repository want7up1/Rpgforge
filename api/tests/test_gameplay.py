...
def test_state_delta_backfills_required_anchor_and_derives_main_quests(db_session) -> None:
            ...
            "completion_anchors": [
                {
                    ...
                    "completion_signal": (
                        "翠屏山废弃雷达站清剿完成，基础防御工事建立，破晓基地初步成型。"
                    ),
                },
                {
    ...
    state.state_json = {
        **state.state_json,
        "location": {
            "current": "破晓基地主楼",
            "known_locations": ["翠屏山废弃雷达站", "破晓基地主楼"],
        },
        "known_facts": [
            "翠屏山废弃雷达站清剿完成，基础防御工事建立，破晓基地初步成型。",
            "破晓基地在主楼内完成幸存者安置和布局确认。",
        ],
    ...
    assert quests["main_quest_1"]["name"] == "建立破晓基地"
    ...
    assert [quest["name"] for quest in quest_log["completed"]] == ["建立破晓基地"]
            ...
            "completion_anchors": [
                {
                    ...
                    "title": "顾倾城臣服",
                    "required": True,
                    "completion_signal": "顾倾城首次臣服，林辰光湮完全觉醒。",
                },
                {
                    "id": "act_3_brotherhood_destroyed",
                    "title": "兄弟会瓦解",
                    "required": True,
                    "completion_signal": "兄弟会势力瓦解，铁狼败走。",
                },
            ],
        }
    ]
    config.story_settings["main_quest_path"] = [
        {
            ...
            "title": "征服顾倾城",
            "objective": "击败并收服顾倾城。",
            "player_visible": "顾倾城仍是当前冲突中心。",
            "completion_signal": "顾倾城首次臣服，林辰光湮觉醒——大范围光能湮灭。",
            "optional": False,
        },
        {
            "id": "main_quest_8",
            ...
            "anchor_history": [
                {
                    ...
                    "reason": "顾倾城首次臣服，光湮完全觉醒。",
                }
            ],
        },
        "quests": [
            {"id": "main_quest_7", "title": "征服顾倾城", "status": "active"},
            {"id": "main_quest_8", "title": "击溃兄弟会", "status": "active"},
...
def test_state_delta_infers_anchor_from_semantic_completion_signal(
            ...
            "completion_anchors": [
                {
                    ...
                    "completion_signal": (
                        "追踪广播信号至研究所顶部，发现叶清璇独自一人在通讯塔广播求援。"
                    ),
                },
                {
                    ...
                    "title": "叶清璇音波异能觉醒",
                    "required": True,
                    "description": "叶清璇的音波异能失控爆发。",
                    "completion_signal": "叶清璇音波异能觉醒，震碎周围玻璃。",
                },
            ],
    ...
    state.state_json = {
        **state.state_json,
        "location": {"current": "保护伞研究所·通讯塔塔顶"},
        "known_facts": [
            (
                "无线电锁定北偏东二十度方向，完整截获叶清璇的求援广播；"
                "信号源为北方研究所废墟通讯塔顶。"
            )
        ],
        "npcs": [
        ...
        "quests": [
            ...
            {"id": "main_quest_11", "title": "稳定叶清璇的音波异能", "status": "hidden"},
        ],
    }
    ...
    config.story_settings["main_quest_path"] = [
        {
            ...
            "act_id": "act_4",
            "title": "探索研究所秘密",
            "objective": "深入研究所地下层获取保护伞实验记录和洛冰加密笔记。",
            "player_visible": "研究所深处似乎隐藏着重要线索。",
            "completion_signal": "L-0421实验体档案和洛冰加密研究笔记获取。",
            "optional": False,
        }
    ]
    game = create_game_from_config(db_session, config)
...
def test_state_delta_advances_ready_act_from_next_act_activity(db_session) -> None:
    ...
    config.story_settings["main_quest_path"] = [
        ...
        },
        {
            ...
            "act_id": "act_2",
            "title": "白小雨能力训练",
            "objective": "训练白小雨对周围情绪的感知。",
            "player_visible": "白小雨需要稳定掌握情绪感知。",
            "completion_signal": "白小雨情绪感知稳定。",
            "optional": False,
        },
        {
            "id": "main_quest_3",
    ...
    config.story_settings["main_quest_path"] = [
        {
            ...
            "act_id": "act_1",
            "title": "营救白小雨",
            "objective": "在废墟中发现并营救白小雨。",
            "player_visible": "附近废墟中似乎有幸存者活动的痕迹。",
            "completion_signal": "成功营救白小雨并建立初步信任。",
            "optional": False,
        }
    ]
    game = create_game_from_config(db_session, config)
...
def test_state_delta_applies_location_to_and_field_patch_updates(db_session) -> None:
    ...
    assert next_state["location"]["current"] == "破晓基地主楼"
    assert "破晓基地主楼" in next_state["location"]["known_locations"]
    assert next_state["v2"]["active_scene"]["location"] == "破晓基地主楼"
    ...
    assert next_state["npcs"][0]["location"] == "破晓基地主楼客厅"
...
def test_state_delta_clears_stale_location_movement_fields(db_session) -> None:
    ...
    assert next_state["location"]["current"] == "破晓基地庭院"
    assert next_state["location"]["region"] == "翠屏山"
    assert "破晓基地庭院" in next_state["location"]["known_locations"]
    assert "from" not in next_state["location"]
    assert "to" not in next_state["location"]
    assert "destination" not in next_state["location"]
    assert "name" not in next_state["location"]
    ...
    assert next_state["v2"]["active_scene"]["location"] == "破晓基地庭院"
...
def test_state_v2_does_not_treat_location_words_as_presence_markers() -> None:
    ...
    assert state["v2"]["active_scene"]["present_npcs"] == ["林辰"]
...
def test_state_delta_applies_nested_changes_without_persisting_changes(db_session) -> None:
    ...
    npc = next_state["npcs"][0]
    assert npc["location"] == "翠屏山破晓基地"
    assert npc["status"] == "已加入破晓基地"
    assert npc["attitude"] == "信任初步建立"
    assert "changes" not in npc
    assert next_state["v2"]["active_scene"]["present_npcs"] == ["苏婉"]
...
def test_state_delta_places_single_brought_npc_at_changed_scene(db_session) -> None:
    ...
    next_state = apply_state_delta(
        ...
        Turn(
            game_id=game.id,
            turn_number=8,
            player_input="单独带苏婉到主堡二层寝宫。",
            gm_output="林辰把苏婉带到主堡二层寝宫，完成体检后让她休息。",
        ),
        {
            "location_change": "翠屏山破晓基地主堡二层寝宫",
            "npc_updates": [
                {
                    "name": "苏婉",
                    "status": "体检后休息",
    ...
    npc = next_state["npcs"][0]
    assert npc["location"] == "翠屏山破晓基地主堡二层寝宫"
    assert next_state["v2"]["active_scene"]["present_npcs"] == ["苏婉"]
...
def test_state_extractor_backfills_location_change_from_scene_heading(db_session) -> None:
    class FakeRouter:
        async def use_flash(self, task_type, messages, **kwargs):
            return ChatCompletionResult(
                content='{"npc_updates":[{"name":"林辰","location":"破晓基地庭院"}]}',
                model="deepseek-flash-test",
                raw={},
            )
    ...
    state.state_json = {
        **state.state_json,
        "location": {
            "current": "南坡西侧猎户小道",
            "known_locations": ["南坡西侧猎户小道", "破晓基地庭院"],
        },
    }
    turn = Turn(
        game_id=game.id,
...
def test_state_extractor_backfills_location_from_hierarchical_scene_heading(
    ...
    class FakeRouter:
        async def use_flash(self, task_type, messages, **kwargs):
            return ChatCompletionResult(
                content='{"npc_updates":[{"name":"苏婉","status":"分析样本"}]}',
                model="deepseek-flash-test",
                raw={},
            )
    ...
    state.state_json = {
        **state.state_json,
        "location": {
            "current": "破晓基地庭院",
            "known_locations": ["破晓基地庭院"],
        },
    }
    turn = Turn(
        game_id=game.id,
        turn_number=72,
        player_input="派苏婉分析金属装置碎片。",
        gm_output="### 副楼·医疗室\n\n苏婉把碎片放在医疗台上分析。",
        visible_summary="苏婉在副楼医疗室分析样本。",
        hidden_summary=None,
        state_delta_json={},
        action_options_json=[],
        model_used="deepseek-v4-pro-test",
...
def test_state_extractor_backfills_location_from_common_npc_location(
        ...
        async def use_flash(self, task_type, messages, **kwargs):
            return ChatCompletionResult(
                content=json.dumps(
                    {
                        "npc_updates": [
                            {"name": "白小雨", "location": "翠屏山·观察点07隧道内"},
                            {"name": "陈雨桐", "location": "翠屏山·观察点07隧道内"},
                            {"name": "苏婉", "location": "翠屏山·观察点07隧道内"},
                        ]
                    },
                    ensure_ascii=False,
                ),
    ...
    state.state_json = {
        **state.state_json,
        "location": {
            "current": "翠屏山·隧道入口（保护伞观察点07）",
            "known_locations": ["翠屏山·隧道入口（保护伞观察点07）"],
        },
        "protagonist": {
    ...
    assert delta["location_change"] == {
        "from": "翠屏山·隧道入口（保护伞观察点07）",
        "to": "翠屏山·观察点07隧道内",
    }
    assert {"name": "林辰", "location": "翠屏山·观察点07隧道内"} in delta[
        "npc_updates"
    ]
    assert next_state["location"]["current"] == "翠屏山·观察点07隧道内"
