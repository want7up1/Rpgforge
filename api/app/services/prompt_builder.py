...
class PromptBuilder:
    def build_runtime_messages(
        ...
        *,
        ...
        runtime_payload = {
            ...
            "current_state": game_state.state_json if game_state else {},
    ...
    def _select_always_on_lore(entries: list[LoreEntry]) -> list[LoreEntry]:
        ...
        always_on = [entry for entry in entries if entry.always_on]
        return sorted(
            always_on,
    ...
    def _campaign_contract_payload(config: GameConfig | None) -> dict[str, Any]:
        ...
        script_outline = config.script_outline if isinstance(config.script_outline, dict) else {}
        worldview = config.worldview if isinstance(config.worldview, dict) else {}

        for key in ("campaign_contract", "director_contract", "story_contract"):
            contract = script_outline.get(key)
        ...
        premise = (
            script_outline.get("title")
            or worldview.get("summary")
            or worldview.get("setting")
            or ""
        )
        return {
            "source": "derived_from_script_outline",
            "premise": premise,
            "tone": worldview.get("tone") or "",
            "act_plan": acts,
            "anchor_rules": [
